...
export class FixturesService {
  ...
  async getFixtureById(fixtureId: number): Promise<Fixture> {
    ...
    try {
      ...
      logger.error({ err, fixtureId }, `Failed to fetch fixture"`);
      throw err;
    }
  ...
  async createNewFixture(payload: InsertFixturePayload): Promise<Fixture> {
    ...
    if (!result) {
      logger.warn(
        { payload },
        `Repository returned empty result when creating new fixture `,
      );
  ...
  async updateFixture(
    ...
    try {
      ...
      // 2. Perform DB update
      const data = await this.repo.updateMatchFixture(
        fixtureId,
        validatedPayload,
      );
      result = (data ?? null) as Fixture | null;
      ...
      logger.error(
        { err, fixtureId },
        `Failed to update fixture with id "${fixtureId}".`,
      );
      throw err;
    }
    ...
    if (!result) {
      logger.warn(
        { fixtureId, updatedData },
        `Failed to update or Fixture not found.`,
      );
      throw new NotFoundError(`Fixture with id "${fixtureId}" not found.`);
    }
