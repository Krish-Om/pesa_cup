...
- [X] **Dockerize S3 storage** — add a [MinIO](https://min.io/) service to `podman-compose.yml` as a local S3-compatible store for development
- [ ] **Update `podman-compose.yml`** — wire the MinIO container to the backend via environment variables and a shared Docker network
